...
export const getInitialValue = (questions: QuestionType[]) => {
  const values: any = {}
  questions.forEach(question => { values[question.name] = question.defaultValue })
  return values
}
  ...
  question.forEach(question => { 
    if(question.validations?.length){
      ...
      question.validations.forEach((validation, index) => {
          schema[question.name] = schema[question.name][validation](index && message[validation](question.label))
      })
    }
  })
